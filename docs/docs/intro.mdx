...
This example uses a JSONPath query to find players with a low score, converts the query results to a `JSONPointer`, then uses that pointer to build a `JSONPatch` that, when applied to our original data, boosts each player's score by 20.
